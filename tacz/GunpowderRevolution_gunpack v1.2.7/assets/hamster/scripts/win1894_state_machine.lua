    ...
    if (main_track_states.reload.loaded_ammo > main_track_states.reload.need_ammo or not context:hasAmmoToConsume()) then
        ...
        if(context:isHolding(track)) then
            if(main_track_states.reload.emptyload == 1) then
                if(context:getMagExtentLevel() == 1 and this.main_track_states.reload.loaded_ammo + 3 <= this.main_track_states.reload.need_ammo) then
                    if(main_track_states.reload.loaded_ammo == 0) then
                        context:runAnimation("reload_loader_empty_start", track, false, PLAY_ONCE_HOLD, 0)
                    else
                        context:runAnimation("reload_loader_empty", track, false, PLAY_ONCE_HOLD, 0)
                    end
                    this.main_track_states.reload.loaded_ammo = this.main_track_states.reload.loaded_ammo + 3
                elseif (main_track_states.reload.loaded_ammo <= main_track_states.reload.need_ammo) then
                    if(main_track_states.reload.loaded_ammo == 0) then
                        context:runAnimation("reload_loop_start", track, false, PLAY_ONCE_HOLD, 0.1)
                    elseif(main_track_states.reload.loaded_ammo == main_track_states.reload.need_ammo - 1) then
                        context:runAnimation("reload_loop_end", track, false, PLAY_ONCE_HOLD, 0.1)
                    else
                        context:runAnimation("reload_loop_empty", track, false, PLAY_ONCE_HOLD, 0.1)
                    end
