...
function main_track_states.reload.update(this, context)
    if (main_track_states.reload.loaded_ammo > main_track_states.reload.need_ammo or not context:hasAmmoToConsume()) then
        ...
        if(context:isHolding(track)) then
            if (context:getAmmoCount() == 0 and context:getMagExtentLevel() == 1) then
                ...
                if (main_track_states.reload.need_ammo - main_track_states.reload.loaded_ammo >= 3) then
                    context:runAnimation("reload_3round", track, false, PLAY_ONCE_HOLD, 0)
                    main_track_states.reload.loaded_ammo = main_track_states.reload.loaded_ammo + 3
                elseif(main_track_states.reload.need_ammo - main_track_states.reload.loaded_ammo >= 2) then
                    context:runAnimation("reload_2round", track, false, PLAY_ONCE_HOLD, 0)
                    main_track_states.reload.loaded_ammo = main_track_states.reload.loaded_ammo + 2
                else
                    context:runAnimation("reload_1round", track, false, PLAY_ONCE_HOLD, 0)
