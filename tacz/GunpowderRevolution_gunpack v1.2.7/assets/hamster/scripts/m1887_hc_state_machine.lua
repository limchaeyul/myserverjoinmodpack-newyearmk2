...
function main_track_states.idle.transition(this, context, input)
    ...
    if (input == INPUT_BOLT) then
        math.randomseed(context:getCurrentTimestamp())
        local rand = math.random()
        if (rand <= 0.6) then
            context:runAnimation("bolt", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_STOP, 0.2)
        else
            context:runAnimation("bolt_rotate", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_STOP, 0.2)
        end
...
function main_track_states.reload.update(this, context)
    if (main_track_states.reload.loaded_ammo > main_track_states.reload.need_ammo or not context:hasAmmoToConsume()) then
        ...
        if(context:isHolding(track)) then
            ...
            if (main_track_states.reload.emptyload == 1) then
                if (main_track_states.reload.loaded_ammo + 1 >= main_track_states.reload.need_ammo) then
                    context:trigger(main_track_states.reload.retreat)
                end
            else
                if (main_track_states.reload.loaded_ammo + 1 >= main_track_states.reload.need_ammo) then
...
function main_track_states.reload.transition(this, context, input)
    if (input == main_track_states.reload.retreat) then
        if (main_track_states.reload.emptyload == 1) then
            if (main_track_states.reload.loaded_ammo + 1 >= main_track_states.reload.need_ammo) then
            ...
            if (main_track_states.reload.need_ammo == 0) then
                context:runAnimation("reload_int", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_STOP, 0.2)
            elseif (main_track_states.reload.loaded_ammo >= main_track_states.reload.need_ammo) then
                context:runAnimation("reload_end_1round", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_STOP, 0.2)
            elseif (main_track_states.reload.loaded_ammo + 1 >= main_track_states.reload.need_ammo) then
                context:runAnimation("reload_end_2round", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_STOP, 0.2)
            end
        end
        return main_track_states.idle
    elseif (input == INPUT_CANCEL_RELOAD) then
        if(main_track_states.reload.emptyload == 1) then
            context:runAnimation("reload_int_empty", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_STOP, 0.2)
            main_track_states.reload.emptyload = 0
        else
            context:runAnimation("reload_int", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_STOP, 0.2)
        end
